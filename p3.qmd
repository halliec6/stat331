...
Each dataset was reshaped so that each row represented a single country-year observation. All duplicate records were removed. The final cleaned dataset contains 9,781 rows, incorporating all meaningful observations while excluding any instance where both sugar and spending values are missing. 
